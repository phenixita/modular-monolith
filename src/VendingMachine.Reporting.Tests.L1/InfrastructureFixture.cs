...
public sealed class InfrastructureFixture : IAsyncLifetime
{
    ...
    private readonly IContainer _postgresContainer = new ContainerBuilder()
        ...
        .WithName($"vendingmachine-reporting-postgres-{Guid.NewGuid():N}")
        .WithEnvironment("POSTGRES_USER", "postgres")
        .WithEnvironment("POSTGRES_PASSWORD", "postgres")
        .WithEnvironment("POSTGRES_DB", "vendingmachine_reporting")
    ...
    public async Task InitializeAsync()
    {
        ...
        ConnectionString =
            $"Host={_postgresContainer.Hostname};Port={_postgresContainer.GetMappedPublicPort(PostgresPort)};Username=postgres;Password=postgres;Database=vendingmachine_reporting;SSL Mode=Disable;Trust Server Certificate=true;Pooling=false";
