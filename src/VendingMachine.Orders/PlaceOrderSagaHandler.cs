...
internal sealed class PlaceOrderSagaHandler(IMediator mediator) : IRequestHandler<PlaceOrderSagaCommand, OrderReceipt>
{
    public async Task<OrderReceipt> Handle(PlaceOrderSagaCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            await RefundChargedCash(product.Price, cancellationToken);
            throw new InvalidOperationException(
                "Order failed while updating stock. Charged cash has been refunded.",
                ex);
        }
    ...
    private async Task RefundChargedCash(decimal amount, CancellationToken cancellationToken) =>
        await mediator.Send(new InsertCashCommand(amount), cancellationToken);
